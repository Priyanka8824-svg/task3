...
function StudentUpdateForm(){
	...
	async function saveStudentsData(data){
		try {
			const res = await AI2.patch(`student/update/${id}/`,data)
			if( res.status == 200 ){
	...
	async function getStudentsData(data){
		
		try {
			const res = await AI2.get(`student/retrieve/${id}/`)
			if( res.status == 200 ){
				console.log('data',res)
				setValue('roll_number', res.data.roll_number )
	...
	return(
		<div className="p-3 mt-2 w-50 mx-auto p-4 ps-5 pe-5 rounded" style={{width:'40%',background:'rgb(170,240,170)'}}>	
			<h4 className="text-center mb-3">student Update Form</h4>
